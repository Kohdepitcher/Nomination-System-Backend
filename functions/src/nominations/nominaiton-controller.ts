...
export class NominationController {
    ...
    */
    ...
    This method will not deny any nominations after the closing time
    as its intended to be used by admins
    */
   async createNominationForMeetingOnBehalfOfTrainer(request: Request, response: Response) {
        
    //body elements
    const { jockey, horseName, horseAge, horseClass, trialMeetingID, trainerID } = request.body;
    ...
    if (!trainerID) {
        return response.status(400).send({ message: 'Missing the ID of the trainer that are being nominated on behalf of' })
    }
    ...
    try {
        ...
        //get the trainer for nomination
        const matchingUser = await userRepo.findOne({ where: { userID: trainerID } })
